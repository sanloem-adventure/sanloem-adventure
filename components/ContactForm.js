...
const ContactForm = () => {
  ...
  return (
    <session id="contact" className="flex flex-col py-10 xl:py-20 bg-secondary gap-10">
      ...
      <div className="flex flex-col xl:flex-row w-full">
        <div className="flex flex-col gap-10 w-full xl:w-7/12">
          <form name="contact" method="POST" data-netlify="true" className="slef-center xl:self-end font-light text-sm flex flex-col gap-6 xl:mr-20 xl:px-0 px-10">
            <label className="flex flex-col xl:flex-row gap-4 w-full xl:justify-end">
              <span className="xl:self-center xl:text-right w-5/12 uppercase">{contact_first_name}</span>
              <input type="text" name="first_name" className="rounded-lg w-full xl:w-7/12 px-4 py-2" />
            </label>
            <label className="flex flex-col xl:flex-row gap-4 w-full justify-end">
              <span className="xl:self-center xl:text-right w-5/12 uppercase">{contact_name_name}</span>
            ...
            <div className="flex flex-col xl:flex-row gap-6 justify-end">
              <div className="w-full xl:w-5/12 flex flex-col gap-6">
                <label className="flex flex-col xl:flex-row xl:grid xl:grid-cols-2 gap-4 justify-end">
                  <span className="xl:w-full w-5/12 xl:self-center xl:text-right">{contact_phone}</span>
                  <input type="text" name="phone" className="w-full rounded-lg px-4 py-2" />
                </label>
                <label className="flex flex-col xl:flex-row xl:grid xl:grid-cols-2 gap-4 justify-end">
                  <span className="xl:w-full w-5/12 xl:self-center xl:text-right">{contact_email}</span>
              ...
              <div className="self-center xl:flex xl:flex-col w-full xl:w-4/12">
                <label className="self-center flex flex-col w-full gap-4">
                  <span className="w-full xl:text-center">LOTS YOU ARE INTERESTED IN</span>
